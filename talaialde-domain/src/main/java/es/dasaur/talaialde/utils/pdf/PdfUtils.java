...
import es.dasaur.talaialde.billing.line.JpaLine.LineType;
...
public class PdfUtils {
    ...
    public static byte[] getBill(JpaClient client, JpaRoute route, JpaTractor tractor,
        ...
        BigDecimal subtotal = lines.stream().map(JpaLine::getValue)
    ...
    public static byte[] getFreeBill(JpaClient client, Date startDate,
                                     Date endDate, List<JpaFreeLine> lines, String vat,
        ...
        BigDecimal subtotal = lines.stream().map(JpaFreeLine::getValue)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        parameters.put("subtotal", 
                Formats.MONEY.format(subtotal.doubleValue()) + " €");
        BigDecimal vatValue = subtotal.multiply(new BigDecimal(vat))
